...
void Worker::repairStripe(int batchid, int stripeid, int tasknum, vector<Task*> tasklist) {
    LOG << "Worker::repairStripe batchid: " << batchid << ", stripeid: " << stripeid << ", tasknum: " << tasknum << endl;
    struct timeval time1, time2, time3, time4, time5, time6;
    gettimeofday(&time1, NULL);
    ...
    LOG << "Worker::repairStripe.realread: " << realread.size() << ", virtread: " << virtread.size() << endl;
    num += realread.size();
    num += virtread.size();
    ...
    for (Task* send: sendtasks) {
        LOG << "Worker::repairStripe.send = " << send->_dagidx_list.size() << endl;
        _sendNum += send->_dagidx_list.size();
    }
    LOG << "Worker::repairStripe.sendNum = " << _sendNum << endl;

    // 3. recv tasks: recv task in the same level are in the same task
    num += recvtasks.size();
    for (Task* recv: recvtasks) {
        LOG << "Worker::repairStripe.recv = " << recv->_dagidx_list.size() << endl;
        _recvNum += recv->_dagidx_list.size();
    }
    LOG << "Worker::repairStripe.recvNum = " << _recvNum << endl;
...
void Worker::readRealBlock(int batchid, int stripeid, Task* task) {
    ...
    LOG << "readRealBlock blkdname = " << blkname  << " Worker::readWorker:fullpath = " << fullpath << endl;
    ...
    for (int i=0; i<pktnum; i++) {
        for (int j=0; j<ecw; j++) {
            ...
            //// debug
            //if (i == 0) {
            //    LOG << "READ dagidx " << dagidx << ": " << (int)curpkt->getData()[0] << endl;
            //}
...
void Worker::sendOneByOne(int batchid, int stripeid, Task* task) {
    ...
    LOG << "sendOneByOne [" << batchid << ":" << stripeid << "] for " << dagidxlist.size() << "pkts from " << getTimeString(time1) << " to " << getTimeString(time2) << endl;
    //LOG << " for ";
...
void Worker::recvOneByOne(int batchid, int stripeid, Task* task) {
    ...
    LOG << "recvOneByOne  " << batchid << ":" << stripeid << " for " << dagidx_list.size()  << "pkts from " << getTimeString(time1) << " to " << getTimeString(time2) << endl;
    //LOG << " for: ";
    //for (auto dagidx: dagidx_list)
    //    LOG << dagidx << " ";
    //LOG << endl;

...
int FlowGraph::getStripeIdWithMinFlow() {
    ...
    for (int i=0; i<_p; i++) {
        int vid = 1+i;
        // 这里的res_matrix中应该是剩余的正向连接
            // 由于连接数量都是1,所以选择的应该是第一个未连接的stripe
        if (_res_matrix[vid] > max) {
            max = _res_matrix[vid];
            id = vid;
        }
    }
    //cout << "vid: " << id << ", stripeid: " << vid2sid[id] << endl;
    return vid2sid[id];
}
...
vector<int> FlowGraph::chooseStripes(int capacity) {
    ...
    for (int vid=1; vid<= _p; vid++) {
        ...
        // cout << "stripedid = " << stripeid <<  "used = " << used << endl;

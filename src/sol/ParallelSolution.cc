...
int ParallelSolution::hungary(int rg_id, int cur_match_stripe, int* matrix, int matrix_start_addr, int* node_selection) {
    ...
    for(i=0; i<_agents_num; i++){ // 遍历每个node
        if((matrix[matrix_start_addr + cur_match_stripe*_agents_num + i]==1) && (_mark[i]==0)){ // 该node是该stripe的存储节点
            if (_debug)
...
int ParallelSolution::if_insert(Stripe* repairstripe, int rg_id, int cur_match_stripe, int soon_to_fail_node) {
    ...
    // soon_to_fail_node fail节点idx
    int chunk_id;
    int k;
    int node_id;
    int ret;

    int* bak_node_belong=(int*)malloc(sizeof(int)*_agents_num); // clusterSize大小
    ...
    for(k=0; k<_agents_num; k++)
        bak_node_belong[k]=_node_belong[rg_id*_agents_num + k]; // 已分配的?
...
vector<RepairBatch*> ParallelSolution::findRepairBatchs(int soon_to_fail_node, string scenario) {
    ...
    while(true){
        ...
        for(i=0; i<num_related_stripes; i++){
            ...
            stripe_id = _stripes_to_repair[i]; // 遍历stripe
            if (_debug)
...
{
    if(DEBUG_ENABLE)
        cout << "ParallelSolution::SingleMLP for stripe " << stripe->getStripeId() << endl;
...
// void ParallelSolution::improve(int fail_node_id, string scenario){
//     cout << "ParallelSolution::improve" << endl;
...
//     for(int i = 0; i < _batch_list.size(); i++)
//     {
//         cout << endl << "[DEBUG] coloring batch: " << i << "/" << _batch_list.size() << endl;
//         RepairBatch* batch = _batch_list[i];
//         vector<vector<int>> loadTable(_agents_num, {0,0}); // {out,in}
...
//             unordered_map<int, int> coloring;
//             genColoringForSingleFailure(stripe, coloring, fail_node_id, _agents_num, scenario, placement);
//             // genOfflineColoringForSingleFailure(stripe, fail_node_id, _agents_num, scenario, loadTable, placement);  
//             stripe->setColoring(coloring);
//             stripe->evaluateColoring();
//             // cout << "init stripe" << stripe->getStripeId() << " load = " << stripe->getLoad() << endl;
//         }


//         // record information
//         table_map.insert(make_pair(batch,loadTable));
//         batch_size_map.insert(make_pair(batch,full_stripe_list.size()));
//         batch->evaluateBatch(_agents_num);
//         // cout << "init batch" << batch->getBatchId() << " load = " << batch->getLoad() << endl;
//     }
//     gettimeofday(&time2, NULL);
//     double color_all_batch = DistUtil::duration(time1, time2);
...
//         vector<pair<Stripe*,RepairBatch*>> record; // for insert entire tail batch into pre batch
//         RepairBatch* tail_batch = _batch_list[last_batch_id];
//         cout << "For batch " << tail_batch->getBatchId() << endl;
...
//             cout << "try to insert stripe" << tail_stripe->getStripeId()<< " into " << tail_batch->getBatchId()<< endl;
...
//                 int loadpre = evalTable(loadTable)._load;
//                 int stripe_num =  batch_size_map[batch];
//                 float avgpre = loadpre*1.0 / stripe_num;
                 
//                 // coloring the solution
//                 unordered_map<int, int> coloring;
//                 genParallelColoringForSingleFailure(tail_stripe, fail_node_id, _agents_num, scenario, loadTable);                
//                 // calculate average load after insert tail stripe
//                 int loadafter = evalTable(loadTable)._load;
//                 float avgafter = loadafter*1.0 / (stripe_num + 1);
                
//                 if(DEBUG_ENABLE){
//                     cout << "try insert stripe " << tail_stripe->getStripeId() << " into batch " << batch->getBatchId()    << endl;
//                     cout << "avg load pre = " << loadpre << "/" << stripe_num << " = " << avgpre << endl;
//                     cout << "avg load after = " << loadafter << "/" << stripe_num+1 << " = " << avgafter << endl;
//                 }
...
//                 if(avgafter < avgpre){
//                     if(DEBUG_ENABLE){
//                         cout << "match insert stripe " << tail_stripe->getStripeId() << " into batch " << batch->getBatchId()    << endl;
//                         cout << "avg load pre = " << loadpre << "/" << stripe_num << " = " << avgpre << endl;
//                         cout << "avg load after = " << loadafter << "/" << stripe_num+1 << " = " << avgafter << endl;
//                     }
//                     batch_idx = i;
...
//             // 3. save in record, entirly insert
//             record.push_back(make_pair(tail_stripe, _batch_list[batch_idx]));
//         }

//         if(!all_optimized)
//         {
//             cout << "coloring vec size = " << coloring_map.size() << endl;
//             cout << "stripelist_size = " << stripe_list.size() << endl;
...
//         // perform insert based on record
//         for(auto pair : record)
//         {
//             RepairBatch* batch = pair.second;
//             Stripe* stripe = pair.first;
//             cout << "perform insert stripe " << stripe->getStripeId() << " into batch " << batch->getBatchId() << endl;
//             batch->push(stripe);
...
//         gettimeofday(&time2, NULL);
//         cout << "duration " << DistUtil::duration(time1, time2) << " for try matching " << debug_try_num << " times "<< endl;
//         cout << "[DEBUG] " << last_batch_id+1 << "/" << right << endl;
//     }
//     cout << "duration first coloring batch = " << color_all_batch << endl;
//     cout << "debug placement2, batch_num = " << _batch_list.size() << endl;
//     dumpPlacement(_batch_list);
// }
...
void ParallelSolution::improve_hungary(int fail_node_id, string scenario){
    ...
    while(true)
    {
        ...
        //             int new_node_load = loadTable[min_load_color][1] + _ec->_w;
        //             if(new_node_load*1.0/(stripe_num+1) > best_avg){
        //                 cout << "new_node input load is to large, = " << new_node_load << " avg=" << new_node_load*1.0/(stripe_num+1) << endl;
        //                 continue;
        //             }      
        //             // choose the maximum outputload amoung stripe source_node
        //             int max_load = 0;
        ...
        //             // cout << "output avg = " << max_gloabl_output << "/" << (stripe_num+1) << "=" << avg_output << endl;
        //             if(avg_output > best_avg){
        //                 // cout << "soucenode output load is to large" << endl;
        //                 continue;
        //             }
        //             find_stripe = true;
        //             stripe = tail_stripe;
        ...
        //     gettimeofday(&time1, NULL);
        //     batch_stripe.push_back(make_pair(batch,stripe));
        //     genParallelColoringForSingleFailure(stripe, fail_node_id, _agents_num, scenario, loadTable);
        //     // calculate average load after insert tail stripe
        //     int loadafter = evalTable(loadTable)._load;
        //     float avgafter = loadafter*1.0 / (stripe_num + 1);
        //     gettimeofday(&time2, NULL);

...
for node in clusternodes:
    ...
    cmd="ssh "+node+" \" wget -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | sudo apt-key add -; sudo apt-get install software-properties-common -y; sudo apt-get update; sudo apt-add-repository \'deb https://apt.kitware.com/ubuntu/ bionic main\' ;sudo apt-get update;sudo apt install cmake -y \""
    print(cmd)
    os.system(cmd)

    cmd="ssh "+node+" \" cd /home/pararc/tools; git clone https://github.com/fpagliughi/sockpp.git; cd sockpp; git checkout v1.x; mkdir build ; cd build; cmake ..; make; sudo make install; sudo ldconfig \""
    print(cmd)
    os.system(cmd)
